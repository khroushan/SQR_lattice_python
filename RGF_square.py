...
for ie in range(N_energy):
    ...
    # the Green's function of the first slice in presence of leads' self-energies
    G_n = lg.inv(energy*I - h - sigma_l)
    sigma_n = np.dot(tau, np.dot(G_n, tau))
    for i_len in range(nlat):
        G_n = lg.inv(energy*I - h - sigma_n)
        sigma_n = np.dot(tau, np.dot(G_n, tau))

    # Full surface Green's function at right lead
    G_d = lg.inv(energy*I - h - sigma_n - sigma_r)
